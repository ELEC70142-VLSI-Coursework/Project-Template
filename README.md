...
6. A folder called 'tapeout' which includes:
    - the final GDSII file needed to be sent to IMEC/Europractice for tapeout. This GDSII file must be extracted from Virtuoso **after** the design has passed DRC and LVS checks with Calibre. 
    - Log files demonstrating successful DRC and LVS runs. Accepted errors are mentioned in the signoff guidelines.
    - The ATPG (Automatic Test Pattern Generation) folder, produced by the `write_dft_atpg` command, provided as a compressed tar archive.
